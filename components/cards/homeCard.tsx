...
const HomeCard: React.FC<HomeCardPorperties> = ({ backgroundImage, title, description, id }) => {
  ...
  return (
    <div className={isDesktop ? "hidden" : "px-5"}>
      <div className="relative gap-y-3 h-[600px] rounded-md py-5 z-0">
        <Image
          src={backgroundImage}
          alt="test"
          width={100}
        ...
        <div className="absolute bottom-5 left-0 w-full h-fit py-5">
          <div className="relative z-10">
            <div className="flex flex-col justify-center items-center px-2 z-20 gap-y-2">
              <p className="h4-hind-m">{title}</p>
              <p className="button-sm">{description}</p>
            </div>
          </div>
          <div className="flex flex-col gap-y-4 px-2 z-20 mt-5">
